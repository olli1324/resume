...
4. Vurder om `index.html` bør laste `data/platform_data.json` med fetch i stedet for inline JSON (3,7 MB) hvis siden føles treg; strukturen er `{researchers, results, patents, spinoffs, themes, depts}` og koden leser den fra `<script id="data" type="application/json">`.
